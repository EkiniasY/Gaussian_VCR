...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            # 后期修剪，默认不启动，启动后会像密集化操作一样，定期修剪透明度过低的高斯点
            ...
            # 冗余点修剪操作，默认不启动，需要手动启动，后期启动
            # 启动条件：在密集化结束后启动，区间(15000, 27000)，每mercy_interval（默认10）*densification_interval（密集化操作，默认100）启动一次
            ...
            (iteration >= opt.densify_until_iter or iteration % opt.opacity_reset_interval != 0):
                try:
                    ...
                    gaussians._cameras = scene.getTrainCameras()
                    scene.gaussians.mercy_points(dens_statistic_dict, args.lambda_mercy, args.mercy_minimum, args.mercy_type)
                except Exception as e:
                    print(f"Warning: calculate_redundancy_metric failed at iteration {iteration}: {e}")
                    print("Skipping mercy pruning this iteration.")
    ...
    if args.residual_quantize:
        print("\n[B2] Cross-attribute prediction residual quantization...")
        scene.gaussians.produce_clusters_residual(k=args.residual_k, store_dict_path=scene.model_path)
        scene.save(iteration, quantise=True)
        print(f"[B2] Residual quantization complete. Saved to {scene.model_path}")

    # B3: Contribution-score-based tiered quantization
    if args.contribution_tiered_quantize:
        print("\n[B3] Contribution-score-based tiered quantization...")
        train_cams = scene.getTrainCameras()
        scene.gaussians.produce_clusters_contribution_tiered(train_cams, store_dict_path=scene.model_path)
        scene.save(iteration, quantise=True)
        print(f"[B3] Tiered quantization complete. Saved to {scene.model_path}")

    # B4: Frequency-based tiered quantization
    if args.frequency_tiered_quantize:
        print("\n[B4] Frequency-based tiered quantization...")
        scene.gaussians.produce_clusters_frequency_tiered(store_dict_path=scene.model_path)
        scene.save(iteration, quantise=True)
        print(f"[B4] Frequency tiered quantization complete. Saved to {scene.model_path}")

    print(f"\nTotal Gaussians: {scene.gaussians.num_primitives}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--residual_quantize", action="store_true", default=False, help="B2: cross-attribute prediction residual quantization")
    parser.add_argument("--residual_k", type=int, default=8, help="B2: KNN k for residual prediction")
    parser.add_argument("--contribution_tiered_quantize", action="store_true", default=False, help="B3: contribution-based tiered quantization")
    parser.add_argument("--frequency_tiered_quantize", action="store_true", default=False, help="B4: SH energy ratio frequency tiered quantization")
