...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def backward(ctx, grad_out_color, _):
        ...
        # Handle P=0 case: return zero gradients matching input shapes
        if means3D.shape[0] == 0:
            P, M = 0, sh.shape[1] if sh.ndim >= 2 else 0
            grads = (
                torch.zeros((P, 3), device=means3D.device),           # grad_means3D
                torch.zeros((P, 3), device=means3D.device),           # grad_means2D
                torch.zeros((P, M, 3), device=sh.device),             # grad_sh
                None,                                                   # degrees
                torch.zeros_like(colors_precomp),                      # grad_colors_precomp
                torch.zeros((P, 1), device=means3D.device),           # grad_opacities
                torch.zeros((P, 3), device=means3D.device),           # grad_scales
                torch.zeros((P, 4), device=means3D.device),           # grad_rotations
                torch.zeros_like(cov3Ds_precomp),                      # grad_cov3Ds_precomp
                None,
                None,
            )
            return grads
