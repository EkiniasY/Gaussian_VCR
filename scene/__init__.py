...
class Scene:
    ...
    def calculate_redundancy_metric(self, pixel_scale=1.0, num_neighbours=30):
        # Guard: skip if too few points remain
        if self.gaussians.num_primitives <= 1:
            return torch.zeros(1, device="cuda"), torch.zeros(1, device="cuda")

        cameras = self.getTrainCameras()
        # Get minimum projected pixel size
        # 确保tensor是连续的，避免CUDA kernel启动参数无效
        means3D = self.gaussians._xyz.contiguous()
        cube_size = find_minimum_projected_pixel_size(
            torch.stack([camera.full_proj_transform for camera in cameras], dim=0).contiguous(),
            torch.stack([camera.inverse_full_proj_transform for camera in cameras], dim=0).contiguous(),
            means3D,
            torch.tensor([camera.image_height for camera in cameras], device="cuda", dtype=torch.int32).contiguous(),
            torch.tensor([camera.image_width for camera in cameras], device="cuda", dtype=torch.int32).contiguous()
        )
